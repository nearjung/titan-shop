...
<table width="1200" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td align="center" valign="top" class="boxmain"> <div class="title">โปรไฟล์ส่วนตัว</div>
      <table width="850">
  <tr>
    <td>
      <div class="animated bounceInLeft">
        <table width="500" border="1" align="center" cellpadding="5" cellspacing="0">
          <tr>
            <td height="26" colspan="2" align="center" bgcolor="#151515"><strong><h3>ข้อมูลส่วนตัว</h3></strong></td>
            </tr>
          <tr>
            <td width="213" align="right" bgcolor="#333333">Play ID :</td>
            <td width="275" bgcolor="#333333"><?php echo $account; ?></td>
            </tr>
          <tr>
            <td align="right" bgcolor="#151515">เพศ :</td>
            <td bgcolor="#151515"><?php echo $api->gander($member['WebSex']); ?></td>
            </tr>
          <tr>
            <td align="right" bgcolor="#333333">อีเมล :</td>
            <td bgcolor="#333333"><?php echo $member['WebEmail']; ?></td>
            </tr>
          <tr>
            <td align="right" bgcolor="#151515">แคชคงเหลือ :</td>
            <td bgcolor="#151515"><?php echo $member['MallMoney']; ?></td>
            </tr>
          <tr>
            <td align="right" bgcolor="#333333">วันที่สมัคร :</td>
            <td bgcolor="#333333"><?php echo $member['id_regdate']; ?></td>
            </tr>
          <tr>
            <td align="right" bgcolor="#151515">เข้าเกมส์ล่าสุด :</td>
            <td bgcolor="#151515"><?php echo $member['game_out_date']; ?></td>
            </tr>
          </table>
        </div>      </td>
  </tr>
  ...
  <tr>
    <td><div class="animated bounceInRight">
      <form name="chgpass" method="post" action=""><table width="500" border="1" align="center" cellpadding="5" cellspacing="0">
        <tr>
          <td height="26" colspan="2" align="center" bgcolor="#151515"><strong>
            <h3>เปลี่ยนรหัสผ่าน</h3>
          </strong></td>
        </tr>
        <tr>
          <td width="213" align="right" bgcolor="#333333">รหัสผ่านเก่า :</td>
          <td width="275" bgcolor="#333333"><label for="oldpass"></label>
            <input type="password" name="oldpass" id="oldpass" /></td>
        </tr>
        <tr>
          <td align="right" bgcolor="#151515">รหัสผ่านใหม่ :</td>
          <td bgcolor="#151515"><label for="newpass1"></label>
            <input type="password" name="newpass1" id="newpass1" /></td>
        </tr>
        <tr>
          <td align="right" bgcolor="#333333">ยืนยันรหัสผ่านใหม่ :</td>
          <td bgcolor="#333333"><label for="newpass2"></label>
            <input type="password" name="newpass2" id="newpass2" /></td>
        </tr>
        <tr>
          <td align="right" bgcolor="#151515">รหัสลับ 4 หลัก :</td>
          <td bgcolor="#151515"><label for="pincode"></label>
            <input name="pincode" type="password" id="pincode" maxlength="4" /></td>
        </tr>
        <tr>
          <td colspan="2" align="center" bgcolor="#333333"><input type="submit" name="changepass" id="changepass" value="เปลี่ยนรหัสผ่านเข้าเกมส์" /></td>
          </tr>
      </table></form>
      <?php
	  if($_POST['changepass'])
	  {
		  ...
		  {
			  $api->popup("กรุณากรอกให้ครบทุกช่องค่ะ");
		  }
		  else if($_POST['newpass1'] != $_POST['newpass2'])
		  {
			  $api->popup("ยืนยันรหัสผ่านใหม่ไม่ตรงกันค่ะ");
		  }
		  else if($_POST['oldpass'] != $member['id_passwd'])
		  {
			  $api->popup("รหัสผ่านเก่าไม่ถูกต้อง");
		  }
		  else if($_POST['pincode'] != $member['WebKey'])
		  {
			  $api->popup("รหัสลับ 4 ตัวไม่ถูกต้องค่ะ");
		  }
		  else
		  {
			  // เปลี่ยนรหัสผ่าน
			  $chgpass_sql = $sql->prepare("EXEC ".MSSQL_DB.".dbo.WebChangePass :p1,:p2");
			  $chgpass_sql->BindParam(":p1",$account);
			  $chgpass_sql->BindParam(":p2",$_POST['newpass2']);
			  $chgpass_sql->execute();
			  $api->popup("ทำการเปลี่ยนรหัสผ่านสำเร็จค่ะ");
			  $api->go("member.php");
		  }
	  }
	  ?>
